...
public class FastApiClient {
    ...
    /* =========================
       >>> 여기가 새로 추가된 부분 <<<
       by-node 조회 (AI/원본 자동 폴백 포함)
       ========================= */

    /**
     * FastAPI: /content/file/by-node 호출 (AI 코드 우선/폴백 포함)
     * - prefer: "auto" | "ai" | "original"
     * - authorizationHeader: null/빈문자열이면 Authorization 헤더 생략
     */
    public ByNodeResponse getFileByNode(
            String nodeKey,
            String filePath,
            String prefer,
        ...
        return webClient.get()
                ...
                .retrieve()
                .bodyToMono(ByNodeResponse.class)
                .timeout(Duration.ofSeconds(30))
                .block();
    }

    /** by-node 호출에서 code 문자열만 뽑아 반환 (없으면 null) */
    public String getCodeByNode(
            String nodeKey,
            String filePath,
            String prefer,
            String authorizationHeader
        ...
        try {
            ByNodeResponse resp = getFileByNode(nodeKey, filePath, prefer, authorizationHeader);
            if (resp != null && StringUtils.hasText(resp.getCode())) {
                return resp.getCode();
            }
            return null;
        } catch (Exception e) {
            // prefer=ai에서 404 등은 여기로 들어옴 → 상위에서 원본 폴백하면 됨
            log.debug("getCodeByNode failed (nodeKey={}, filePath={}, prefer={}): {}",
                    nodeKey, filePath, prefer, e.getMessage());
            return null;
        }
    }
    ...
    /* >>> 새 DTO 추가 <<< */
    @Getter @Setter
    public static class ByNodeResponse {
        @JsonProperty("node_key")
        private String nodeKey;
        @JsonProperty("map_id")
        private String mapId;
        private String path;
        private String origin;         // "ai" | "original"
        @JsonProperty("suggestion_key")
        private String suggestionKey;  // 있을 때만 존재
        private String code;           // 본문 코드
